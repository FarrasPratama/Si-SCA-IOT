...
//  Serial.println(diff);
  ...
  beats[beat_Index] = currentBPM;  // store to array to convert the average
  float total = 0.0;
  for (int i = 0; i < 4; i++){
    total += beats[i];
  }
...
//  Serial.println(HR_Signal);
  ...
  data = String(HR_Signal) + "," + String(BPM) + "," + String(diff); 
  delay(1);
}
...
bool Defibrilator::IsAritmia(){
  ...
  if(_HBdetected){
    ...
    else if ((RRArray[1] < RRArray[0]) && (RRArray[0] < RRArray[2])){
      Serial.println(1);
      isaritmia = true;
      digitalWrite(BUZZER, HIGH);
    }
    else if (RRArray[1] > RRArray[0]){
      Serial.println(1);
      isaritmia = true;
      digitalWrite(BUZZER, HIGH);
    }
    else{
      Serial.println("Normal Gess OOO");
      isaritmia = false;
    }
...
void Defibrilator::Defibrilate(){
    ...
    {    
        ...
        if(_Defon && this->_relaystate == HIGH){
            _Defon = false;
            RRDetection = "NORMAL";
        }
    }
...
void Defibrilator::Ledact(uint16_t HB){
  float percentage = (HB*100)/(this->_MaxHR);
